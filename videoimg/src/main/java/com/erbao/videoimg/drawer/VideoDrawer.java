package com.erbao.videoimg.drawer;
...
import com.erbao.videoimg.filter.AFilter;
import com.erbao.videoimg.filter.GroupFilter;
import com.erbao.videoimg.filter.NoFilter;
import com.erbao.videoimg.filter.ProcessFilter;
import com.erbao.videoimg.filter.RotationOESFilter;
import com.erbao.videoimg.filter.WaterMarkFilter;
import com.erbao.videoimg.gpufilter.SlideGpuFilterGroup;
import com.erbao.videoimg.gpufilter.basefilter.ChangedtoFilter;
import com.erbao.videoimg.gpufilter.basefilter.GPUImageFilter;
import com.erbao.videoimg.gpufilter.filter.MagicBeautyFilter;
import com.erbao.videoimg.media.VideoInfo;
import com.erbao.videoimg.utils.EasyGlUtils;
import com.erbao.videoimg.utils.MatrixUtils;
...
public class VideoDrawer implements GLSurfaceView.Renderer {
    ...
    private ChangedtoFilter mChangedtoFilter;
    ...
    public VideoDrawer(Context context,Resources res){
        ...
        mChangedtoFilter = new ChangedtoFilter();
...
//        mShow.setMatrix(OM);
    ...
    @Override
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {//用于初始化操作
        ...
        mChangedtoFilter.init();
    }
    public void onVideoinvalidate(VideoInfo info){
    ...
    @Override
    public void onSurfaceChanged(GL10 gl, int width, int height) {//尺寸发生变化时调用的方法
        viewWidth=width;
        viewHeight=height;
        ...
        mChangedtoFilter.onSizeChanged(viewWidth,viewHeight);
    }

    @Override
    public void onDrawFrame(GL10 gl) {//具体的绘制操作
        ...
        mChangedtoFilter.onDrawFrame(mProcessFilter.getOutputTexture());
        if (mGroupFilter != null){
            EasyGlUtils.bindFrameTexture(fFrame[0],fTexture[0]);
            GLES20.glViewport(0,0,viewWidth,viewHeight);
            mGroupFilter.onDrawFrame(mSlideFilterGroup.getOutputTexture());
            mGroupFilter.onDrawFrame(mChangedtoFilter.getOutputTexture());
            EasyGlUtils.unBindFrameBuffer();
            mProcessFilter.setTextureId(fTexture[0]);
        }else {
            mProcessFilter.setTextureId(mSlideFilterGroup.getOutputTexture());
            mProcessFilter.setTextureId(mChangedtoFilter.getOutputTexture());
    ...
    public void setChangedtoFilterGPUImageFilter(GPUImageFilter filter){
        mChangedtoFilter.setGPUImageFilter(filter);
