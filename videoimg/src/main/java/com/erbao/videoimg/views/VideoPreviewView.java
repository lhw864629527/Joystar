package com.erbao.videoimg.views;
...
import com.erbao.videoimg.drawer.VideoDrawer;
...
//                mDrawer.onVideoChanged(info);
                mDrawer.onVideoinvalidate(info);
            }
        });
        if(callback!=null){
     ...
     * 设置video的音量
     * */
    public void setVideoVolume(float volem){
        mMediaPlayer.setVolume(volem);
    }
    ...
    /**设置滤镜效果
     * */
    public void setChangedtoFilterGPUImageFilter(final GPUImageFilter filter){
        queueEvent(new Runnable() {
            @Override
            public void run() {
                mDrawer.setChangedtoFilterGPUImageFilter(filter);
            }
        });
